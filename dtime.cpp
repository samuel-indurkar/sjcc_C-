...
void DigitalTime::interval_since(const DigitalTime& previous_time,
                 int& hours_in_interval,  int& minutes_in_interval)  const
{
    // you need to complete this part of the program to compute the
    // a)  hours_in_interval, and
    // b)  minutes_in_interval

    int temp_minutes;
    int current_time_in_min;
    int prev_time_in_min;

    prev_time_in_min = (previous_time.hour *60) + previous_time.minute;
    //cout << "Prev inn min  " << prev_time_in_min << endl;

    current_time_in_min = (hour *60) + minute;
    //cout << "current in min" << current_time_in_min<<endl;

    if (prev_time_in_min < current_time_in_min)
    {
    	temp_minutes = current_time_in_min - prev_time_in_min;
    	//cout << temp_minutes;
    	hours_in_interval = temp_minutes/60;
    	minutes_in_interval = temp_minutes%60;
    }
    else
    {
    	temp_minutes = 1440 - prev_time_in_min;
    	temp_minutes += current_time_in_min;
    	hours_in_interval = temp_minutes/60;
    	minutes_in_interval = temp_minutes%60;
    }
